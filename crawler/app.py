...
async def get_jobs(
    ...
    if source == "jobkorea":
        ...
        return crawl_saramin_data(keyword, page, page_size)
    elif source == "worknet":
...
async def get_integrated_jobs(
    ...
    # Saramin 크롤링
    try:
        saramin_data = crawl_saramin_data(keyword, page, page_size)
...
@fast_app.get("/debug/jobkorea/detail")
async def debug_jobkorea_detail(url: str):
    """
    JobKorea 상세 디버그 API
    실제 크롤링 결과를 그대로 반환합니다.
    """
    try:
        result = await crawl_job_detail_async(url)
...
@fast_app.get("/debug/detail")
async def debug_detail(url: str):
    detail = await crawl_worknet_detail(url)
